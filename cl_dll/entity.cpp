...
// Randomized Gunsmoke typeshi
void UpdateMuzzleSmokePosition(TEMPENTITY* tempEnt, float frametime, float currenttime)
{
	cl_entity_t* localPlayer = gEngfuncs.GetLocalPlayer();
	if (localPlayer)
	{
		Vector muzzlePos = localPlayer->attachment[0]; // Assuming attachment[0] is the muzzle
		VectorSubtract(muzzlePos, localPlayer->origin, tempEnt->tentOffset);
	}
}

void EV_GunsmokeEffect(Vector muzzlePosition, Vector dir)
{
	// Randomize whether the effect should appear
	if (gEngfuncs.pfnRandomFloat(0.0f, 1.0f) > 0.3f) // 70% chance to skip the effect
	{
		return;
	}
	...
	TEMPENTITY* ent = gEngfuncs.pEfxAPI->R_TempSprite(smokeOrigin, velocity, gEngfuncs.pfnRandomFloat(0.1f, 0.2f), spriteIndex, kRenderTransAdd, 0, 0.3f, size, FTENT_SPRANIMATE | FTENT_FADEOUT | FTENT_COLLIDEKILL | FTENT_PLYRATTACHMENT);

	if (ent)
	{
		ent->entity.curstate.renderamt = 20.f;
		ent->fadeSpeed = gEngfuncs.pfnRandomFloat(500.0f, 750.0f);
		ent->entity.curstate.framerate = gEngfuncs.pfnRandomFloat(40.0f, 55.0f);

		ent->clientIndex = player->index;
		VectorSubtract(muzzlePosition, player->origin, ent->tentOffset);

		ent->callback = UpdateMuzzleSmokePosition;
	}
}
...
// Experiment
			...
			if (gEngfuncs.PM_PointContents(pTemp->entity.origin, 0) == CONTENTS_WATER)
			{
				...
				pTemp->entity.baseline.origin[2] = min(pTemp->entity.baseline.origin[2], 5);
			}
			else
			{
