...
extern Vector CalculateBloodDirection(entvars_t* pevVictim, const TraceResult* ptr, const Vector& attackDir);
...
//			ALERT( at_console, "%s : %f\n", STRING( pObject->pev->classname ), flDot );
	...
	if (pObject )
	{
		...
		else if (((pev->button & IN_USE) != 0 && (caps & FCAP_CONTINUOUS_USE) != 0) ||( (m_afButtonPressed & IN_USE) && (caps & (FCAP_IMPULSE_USE|FCAP_ONOFF_USE)) ) )
		{
...
int CBasePlayer::AddPlayerItem( CBasePlayerItem *pItem )
{
	...
	if (pItem->AddToPlayer( this ))
	{
		...
		if (CVAR_GET_FLOAT("sv_hev_pick") == 1)
		{
